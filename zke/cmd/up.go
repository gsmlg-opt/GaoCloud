...
func ClusterUpForGaoCloud(ctx context.Context, clusterState *core.FullState, dialersOptions hosts.DialersOptions) (*core.FullState, error) {
	kubeCluster, err := core.InitClusterObject(ctx, clusterState.DesiredState.ZKEConfig.DeepCopy())
	if err != nil {
	...
	clusterState, err = kubeCluster.UpdateClusterCurrentStateForGaoCloud(ctx, clusterState)
	if err != nil {
		return clusterState, err
	}
...
func ClusterUpFromGaoCloud(scCtx context.Context, zkeConfig *types.ZKEConfig, clusterState *core.FullState, logger cementlog.Logger) (*core.FullState, error) {
	startUPtime := time.Now()
	...
	newClusterState, err := ClusterInitForGaoCloud(ctx, zkeConfig, clusterState, hosts.DialersOptions{})
	if err != nil {
		return clusterState, err
	}

	newClusterState, err = ClusterUpForGaoCloud(ctx, newClusterState, hosts.DialersOptions{})
