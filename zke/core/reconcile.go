...
func reconcileWorker(ctx context.Context, currentCluster, kubeCluster *Cluster, kubeClient *kubernetes.Clientset) error {
	...
	default:
		...
		for _, toDeleteHost := range wpToDelete {
			...
			if err := reconcileHost(ctx, toDeleteHost, true, false, currentCluster.Image.Alpine, currentCluster.Image.ZKERemover, currentCluster.DockerDialerFactory, currentCluster.PrivateRegistriesMap, currentCluster.Option.PrefixPath, currentCluster.Option.KubernetesVersion, currentCluster.Option.ClusterCidr); err != nil {
...
func reconcileHost(ctx context.Context, toDeleteHost *hosts.Host, worker, etcd bool, cleanerImage, storageCleanerImage string, dialerFactory hosts.DialerFactory, prsMap map[string]types.PrivateRegistry, clusterPrefixPath string, clusterVersion string, clusterCIDR string) error {
	...
	if worker {
		...
		if err := toDeleteHost.CleanUpWorkerHost(ctx, cleanerImage, storageCleanerImage, prsMap, clusterCIDR); err != nil {
		...
		if err := toDeleteHost.CleanUpEtcdHost(ctx, cleanerImage, storageCleanerImage, prsMap, clusterCIDR); err != nil {
			return fmt.Errorf("Not able to clean the host: %v", err)
		}
	} else {
		...
		if err := toDeleteHost.CleanUpControlHost(ctx, cleanerImage, storageCleanerImage, prsMap, clusterCIDR); err != nil {
			return fmt.Errorf("Not able to clean the host: %v", err)
		}
	}
	return nil
...
func reconcileEtcd(ctx context.Context, currentCluster, kubeCluster *Cluster, kubeClient *kubernetes.Clientset) error {
	...
	default:
		...
		for _, etcdHost := range etcdToDelete {
			...
			// attempting to clean services/files on the host
			if err := reconcileHost(ctx, etcdHost, false, true, currentCluster.Image.Alpine, currentCluster.Image.ZKERemover, currentCluster.DockerDialerFactory, currentCluster.PrivateRegistriesMap, currentCluster.Option.PrefixPath, currentCluster.Option.KubernetesVersion, currentCluster.Option.ClusterCidr); err != nil {
...
func cleanControlNode(ctx context.Context, kubeCluster, currentCluster *Cluster, toDeleteHost *hosts.Host) error {
	...
	// attempting to clean services/files on the host
	if err := reconcileHost(ctx, toDeleteHost, false, false, currentCluster.Image.Alpine, currentCluster.Image.ZKERemover, currentCluster.DockerDialerFactory, currentCluster.PrivateRegistriesMap, currentCluster.Option.PrefixPath, currentCluster.Option.KubernetesVersion, currentCluster.Option.ClusterCidr); err != nil {
